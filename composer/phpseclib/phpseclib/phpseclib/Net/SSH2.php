...
namespace OCA\Libresign\Vendor\phpseclib4\Net;

use OCA\Libresign\Vendor\phpseclib4\Common\Functions\Strings;
use OCA\Libresign\Vendor\phpseclib4\Crypt\Blowfish;
use OCA\Libresign\Vendor\phpseclib4\Crypt\ChaCha20;
use OCA\Libresign\Vendor\phpseclib4\Crypt\DH;
use OCA\Libresign\Vendor\phpseclib4\Crypt\DSA;
use OCA\Libresign\Vendor\phpseclib4\Crypt\EC;
use OCA\Libresign\Vendor\phpseclib4\Crypt\Hash;
use OCA\Libresign\Vendor\phpseclib4\Crypt\RC4;
use OCA\Libresign\Vendor\phpseclib4\Crypt\RSA;
use OCA\Libresign\Vendor\phpseclib4\Crypt\Rijndael;
use OCA\Libresign\Vendor\phpseclib4\Crypt\TripleDES;
use OCA\Libresign\Vendor\phpseclib4\Crypt\Twofish;
use OCA\Libresign\Vendor\phpseclib4\Crypt\Common\PrivateKey;
use OCA\Libresign\Vendor\phpseclib4\Crypt\Common\SymmetricKey;
...
use OCA\Libresign\Vendor\phpseclib4\Exception\InvalidModeException;
use OCA\Libresign\Vendor\phpseclib4\Exception\InvalidPacketLengthException;
...
use OCA\Libresign\Vendor\phpseclib4\Exception\NoSupportedAlgorithmsException;
use OCA\Libresign\Vendor\phpseclib4\Exception\ServiceUnavailableException;
use OCA\Libresign\Vendor\phpseclib4\Exception\TimeoutException;
use OCA\Libresign\Vendor\phpseclib4\Exception\UnexpectedSSHMessageException;
use OCA\Libresign\Vendor\phpseclib4\Exception\UnexpectedValueException;
use OCA\Libresign\Vendor\phpseclib4\Exception\UnsupportedAlgorithmException;
...
use OCA\Libresign\Vendor\phpseclib4\Net\SSH2\ChannelConnectionFailureReason;
use OCA\Libresign\Vendor\phpseclib4\Net\SSH2\DisconnectReason;
use OCA\Libresign\Vendor\phpseclib4\Net\SSH2\MessageType;
use OCA\Libresign\Vendor\phpseclib4\Net\SSH2\MessageTypeExtra;
use OCA\Libresign\Vendor\phpseclib4\Net\SSH2\TerminalMode;
use OCA\Libresign\Vendor\phpseclib4\System\SSH\Agent;
/**
 * Pure-PHP implementation of SSHv2.
 *
 * @author  Jim Wigginton <terrafrost@php.net>
...
class SSH2
{
    ...
    public const MASK_CONSTRUCTOR = 0x1;
    public const MASK_CONNECTED = 0x2;
    public const MASK_LOGIN_REQ = 0x4;
    public const MASK_LOGIN = 0x8;
    public const MASK_SHELL = 0x10;
    public const MASK_DISCONNECT = 0x20;
    /*
     * Channel constants
     *
     ...
     * @see \phpseclib4\Net\SSH2::getLog()
     */
    public const LOG_MAX_SIZE = 1048576;
    // 1024 * 1024
    /**
     ...
     */
    private bool $send_id_string_first = \true;
    /**
     * Send the key exchange initiation packet first?
     */
    private bool $send_kex_first = \true;
    /**
    ...
    private function key_exchange(?string $kexinit_payload_server = null) : bool
    {
        ...
        $kex_algorithms = $preferred['kex'] ?? SSH2::getSupportedKEXAlgorithms();
        $server_host_key_algorithms = $preferred['hostkey'] ?? SSH2::getSupportedHostKeyAlgorithms();
        $s2c_encryption_algorithms = $preferred['server_to_client']['crypt'] ?? SSH2::getSupportedEncryptionAlgorithms();
        $c2s_encryption_algorithms = $preferred['client_to_server']['crypt'] ?? SSH2::getSupportedEncryptionAlgorithms();
        $s2c_mac_algorithms = $preferred['server_to_client']['mac'] ?? SSH2::getSupportedMACAlgorithms();
        $c2s_mac_algorithms = $preferred['client_to_server']['mac'] ?? SSH2::getSupportedMACAlgorithms();
        $s2c_compression_algorithms = $preferred['server_to_client']['comp'] ?? SSH2::getSupportedCompressionAlgorithms();
        $c2s_compression_algorithms = $preferred['client_to_server']['comp'] ?? SSH2::getSupportedCompressionAlgorithms();
        ...
        if (\str_starts_with($this->kex_algorithm, 'curve25519-sha256') || \str_starts_with($this->kex_algorithm, 'ecdh-sha2-nistp')) {
            $curve = \str_starts_with($this->kex_algorithm, 'curve25519-sha256') ? 'Curve25519' : \substr($this->kex_algorithm, 10);
    ...
    {
        return match ($algorithm) {
            'arcfour256', 'aes192-ctr', 'aes256-ctr' => \true,
            default => \false,
        };
    }
    /**
     ...
     */
    private function login_helper(string $username, #[\SensitiveParameter] string|PrivateKey|array|Agent|null $password = null) : bool
    {
            ...
            case MessageType::USERAUTH_INFO_REQUEST:
                ...
                if (isset($this->keyboard_requests_responses)) {
                    for ($i = 0; $i < $num_prompts; $i++) {
                        [$prompt] = Strings::unpackSSH2('sC', $response);
    ...
    private function privatekey_login(string $username, #[\SensitiveParameter] PrivateKey $privatekey) : bool
    {
        ...
        $this->send_binary_packet($packet);
        $response = $this->get_binary_packet_or_close(MessageType::USERAUTH_SUCCESS, MessageType::USERAUTH_FAILURE, MessageTypeExtra::USERAUTH_PK_OK);
        [$type] = Strings::unpackSSH2('C', $response);
        switch ($type) {
            case MessageType::USERAUTH_FAILURE:
                [$auth_methods] = Strings::unpackSSH2('L', $response);
        ...
        $this->send_binary_packet($packet);
        $response = $this->get_binary_packet_or_close(MessageType::USERAUTH_SUCCESS, MessageType::USERAUTH_FAILURE);
        [$type] = Strings::unpackSSH2('C', $response);
        switch ($type) {
            case MessageType::USERAUTH_SUCCESS:
                $this->bitmap |= self::MASK_LOGIN;
    ...
    protected function open_channel(int $channel, bool $skip_extended = \false) : bool
    {
        ...
        $packet_size = 0x4000;
        $packet = Strings::packSSH2('CsN3', MessageType::CHANNEL_OPEN, 'session', $channel, $this->window_size_server_to_client[$channel], $packet_size);
        $this->send_binary_packet($packet);
    ...
    private function get_binary_packet_or_close(int ...$message_types) : string
    {
        try {
            ...
            if (\count($message_types) > 0 && !\in_array(\ord($packet[0]), $message_types)) {
                $this->disconnect_helper(DisconnectReason::PROTOCOL_ERROR);
                throw new UnexpectedSSHMessageException('Bad message type. Expected: #' . \implode(', #', $message_types) . '. Got: #' . \ord($packet[0]));
            }
            return $packet;
    ...
    private function get_binary_packet() : string
    {
        ...
        if (\defined('NET_SSH2_LOGGING')) {
            ...
            $message_number = \sprintf('<- %s (since last: %s, network: %ss)', ($constantName = MessageType::findConstantNameByValue($value = \ord($payload[0]))) ? "SSH_MSG_{$constantName}" : "UNKNOWN ({$value})", \round($current - $this->last_packet, 4), \round($packet->read_time, 4));
    ...
    private function handleDisconnect($payload) : void
    {
        ...
        $this->errors[] = 'SSH_MSG_DISCONNECT: ' . DisconnectReason::findConstantNameByValue($reason_code) . "\r\n{$message}";
        $this->disconnect_helper(DisconnectReason::CONNECTION_LOST);
        throw new ConnectionClosedException('Connection closed by server');
    }
    /**
            ...
            switch (\ord($payload[0])) {
                case MessageType::CHANNEL_REQUEST:
                    if (\strlen($payload) == 31) {
                        ['channel' => $channel, 'length' => $length] = \unpack('cpacket_type/Nchannel/Nlength', $payload);
                    ...
                    switch ($data) {
                        ...
                        case 'auth-agent@openssh.com':
                            if (isset($this->agent)) {
                                ...
                                [$remote_window_size, $remote_maximum_packet_size] = Strings::unpackSSH2('NN', $payload);
                    ...
                    case MessageType::CHANNEL_OPEN:
                        switch ($type) {
                            case MessageType::CHANNEL_OPEN_CONFIRMATION:
                                [$this->server_channels[$channel], $window_size, $this->packet_size_client_to_server[$channel]] = Strings::unpackSSH2('NNN', $response);
                                if ($window_size < 0) {
     ...
     * @see self::_get_binary_packet()
     */
    protected function send_binary_packet(#[\SensitiveParameter] string $data, ?string $logged = null) : void
    {
     ...
     */
    protected function send_channel_packet(int $client_channel, #[\SensitiveParameter] string $data) : void
    {
        if (isset($this->channel_buffers_write[$client_channel]) && \str_starts_with($data, $this->channel_buffers_write[$client_channel])) {
    ...
    public static function getSupportedEncryptionAlgorithms() : array
    {
        ...
        if (self::$crypto_engine) {
            ...
            $engines = ['libsodium', 'OpenSSL (AEAD)', 'OpenSSL', 'Eval', 'PHP'];
        }
        $ciphers = [];
    ...
    public function getServerPublicHostKey() : ?string
    {
        ...
        if ($this->signature_validated) {
            return $this->bitmap ? $this->signature_format . ' ' . $server_public_host_key : null;
        }
