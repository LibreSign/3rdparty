...
use OCA\Libresign\Vendor\phpseclib4\Common\Functions\Files;
...
use OCA\Libresign\Vendor\phpseclib4\Exception\FileSystemException;
...
use OCA\Libresign\Vendor\phpseclib4\Exception\UnexpectedSFTPPacketException;
...
use OCA\Libresign\Vendor\phpseclib4\Net\SFTP\Attribute;
use OCA\Libresign\Vendor\phpseclib4\Net\SFTP\FileType;
use OCA\Libresign\Vendor\phpseclib4\Net\SFTP\OpenFlag;
use OCA\Libresign\Vendor\phpseclib4\Net\SFTP\OpenFlag5;
use OCA\Libresign\Vendor\phpseclib4\Net\SFTP\PacketType as SFTPPacketType;
use OCA\Libresign\Vendor\phpseclib4\Net\SFTP\StatusCode;
use OCA\Libresign\Vendor\phpseclib4\Net\SSH2\MessageType as SSH2MessageType;
/**
...
class SFTP extends SSH2
{
     ...
     */
    private bool $use_stat_cache = \true;
    /**
     * Sort Options
     *
     ...
     * @see self::realpath()
     */
    private bool $canonicalize_paths = \true;
    /**
     * Request Buffers
     *
     * @see self::_get_sftp_packet()
    ...
    private function partial_init_sftp_connection() : void
    {
        ...
        $this->send_sftp_packet(SFTPPacketType::INIT, "\x00\x00\x00\x03");
            ...
            foreach ($supported as $ver) {
                if (\in_array($ver, $versions)) {
                    ...
                    if ($status != StatusCode::OK) {
                        ...
                        // FileSystemException
                        throw new UnexpectedValueException('Expected SSH_FX_OK. Got ' . StatusCode::getConstantNameByValue($status));
                    }
                    break;
                }
            }
    ...
    {
        try {
            $files = $this->readlist($dir, \true, $onFile);
        } catch (FileSystemException $e) {
            if (!$recursive) {
                throw $e;
            }
        ...
        foreach ($files as $key => $value) {
            ...
            if ($is_directory) {
                $depth++;
                $files[$key] = $this->rawlist($dir . '/' . $key, \true, $onFile);
                $depth--;
            } else {
     ...
     */
    private function readlist(string $dir, bool $raw = \true, ?\Closure $onFile = null) : array
    {
        $this->precheck();
        $dir = $this->realpath($dir . '/');
        ...
        while (\true) {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if (isset($attributes['type']) && $attributes['type'] == FileType::DIRECTORY && ($shortname != '.' && $shortname != '..')) {
                    ...
                    $temp = $shortname == '..' ? $this->realpath($dir . '/..') . '/.' : $dir . '/' . $shortname;
    ...
    private function mkdir_helper(string $dir, int $mode) : void
    {
        ...
        $this->send_sftp_packet(SFTPPacketType::MKDIR, Strings::packSSH2('s', $dir) . "\x00\x00\x00\x00");
        $this->get_sftp_packet_or_error([SFTPPacketType::STATUS], StatusCode::OK);
        if ($mode !== -1) {
            $this->chmod($dir, $mode);
        }
     ...
     */
    public function put(string $remote_file, #[\SensitiveParameter] mixed $data, int $mode = self::SOURCE_STRING, int $start = -1, int $local_start = -1, ?\Closure $progressCallback = null) : void
    {
        $this->precheck();
    ...
    private function read_responses(int $i, ?array $files = null) : void
    {
        ...
        while ($i-- > 0) {
            ...
            if ($this->packet_type != SFTPPacketType::STATUS) {
                throw new UnexpectedValueException('Expected SSH_FXP_STATUS. Got packet type: SSH_FXP_' . SFTPPacketType::getConstantNameByValue($this->packet_type));
            }
            [$status] = Strings::unpackSSH2('N', $response);
            if ($status != StatusCode::OK) {
                if ($files === null) {
                ...
                switch ($this->packet_type) {
                    ...
                    default:
                        ...
                        if ($this->channel_close) {
                            ...
                            throw new UnexpectedSFTPPacketException('Expected SSH_FXP_DATA or SSH_FXP_STATUS. ' . 'Got packet type: SSH_FXP_' . SFTPPacketType::getConstantNameByValue($this->packet_type));
                        }
                }
                unset($response);
            }
     ...
     */
    public function delete(string $path, bool $recursive = \true) : void
    {
        $this->precheck();
        if (!\strlen($path)) {
    ...
    private static function parseMode(int $mode) : ?int
    {
        ...
        return match ($mode & 0170000) {
            ...
            040000 => FileType::DIRECTORY,
            0100000 => FileType::REGULAR,
            0120000 => FileType::SYMLINK,
            // new types introduced in SFTPv5+
            // http://tools.ietf.org/html/draft-ietf-secsh-filexfer-05#section-5.2
            010000 => FileType::FIFO,
            // named pipe (fifo)
            020000 => FileType::CHAR_DEVICE,
            // character special
            060000 => FileType::BLOCK_DEVICE,
            // block special
            0140000 => FileType::SOCKET,
            // socket
            // "SPECIAL should be used for files that are of
            //  a known type which cannot be expressed in the protocol"
            0160000 => FileType::SPECIAL,
            // whiteout
    ...
    private function send_sftp_packet(int $type, #[\SensitiveParameter] string $data, int $request_id = 1) : void
    {
        ...
        if (\defined('NET_SFTP_LOGGING')) {
            $packet_type = '-> SSH_FXP_' . SFTPPacketType::getConstantNameByValue($type) . ' (' . \round($stop - $start, 4) . 's)';
    ...
    private function get_sftp_packet(?int $request_id = null) : string
    {
        ...
        if ($this->use_request_id) {
            ['packet_id' => $packet_id] = \unpack('Npacket_id', Strings::shift($this->packet_buffer, 4));
        ...
        if (\defined('NET_SFTP_LOGGING')) {
            $packet_type = '<- SSH_FXP_' . SFTPPacketType::getConstantNameByValue($this->packet_type) . ' (' . \round($stop - $start, 4) . 's)';
    ...
    public function copy(string $oldname, string $newname) : void
    {
        ...
        if (!isset($this->extensions['copy-data']) || $this->extensions['copy-data'] !== '1') {
            throw new ServiceUnavailableException("Extension 'copy-data' is not supported by the server. " . "Call getSupportedVersions() to see a list of supported extension");
        }
        ...
        $flags = $this->version >= 5 ? OpenFlag5::OPEN_OR_CREATE : OpenFlag::WRITE | OpenFlag::CREATE;
    ...
    public function posix_rename(string $oldname, string $newname) : void
    {
        ...
        if ($this->version >= 5) {
            ...
            throw new ServiceUnavailableException("Extension 'posix-rename@openssh.com' is not supported by the server. " . "Call getSupportedVersions() to see a list of supported extension");
        }
        $this->get_sftp_packet_or_error([SFTPPacketType::STATUS], StatusCode::OK);
        // don't move the stat cache entry over since this operation could very well change the
        // atime and mtime attributes
    ...
    public function statvfs(string $path) : array
    {
        ...
        if (!isset($this->extensions['statvfs@openssh.com']) || $this->extensions['statvfs@openssh.com'] !== '2') {
            throw new ServiceUnavailableException("Extension 'statvfs@openssh.com' is not supported by the server. " . "Call getSupportedVersions() to see a list of supported extension");
        }
        ...
        return ['bsize' => $v[0], 'frsize' => $v[1], 'blocks' => $v[2], 'bfree' => $v[3], 'bavail' => $v[4], 'files' => $v[5], 'ffree' => $v[6], 'favail' => $v[7], 'fsid' => $v[8], 'flag' => $v[9], 'namemax' => $v[10]];
    }
    /** @psalm-suppress PossiblyUnusedMethod */
    public function hardlink(string $oldpath, string $newpath) : void
    {
        ...
        if (!isset($this->extensions['hardlink@openssh.com']) || $this->extensions['hardlink@openssh.com'] !== '1') {
            throw new ServiceUnavailableException("Extension 'hardlink@openssh.com' is not supported by the server. " . "Call getSupportedVersions() to see a list of supported extension");
        }
    ...
    private function get_sftp_packet_or_error(array $packet_types, ?int $expectedStatus = null) : string
    {
        ...
        if (\count($packet_types) > 0 && !\in_array($this->packet_type, $packet_types)) {
            ...
            throw new UnexpectedSFTPPacketException("Expected {$expected}. Got packet type: SSH_FXP_" . SFTPPacketType::getConstantNameByValue($this->packet_type));
        }
        if ($this->packet_type == SFTPPacketType::STATUS) {
