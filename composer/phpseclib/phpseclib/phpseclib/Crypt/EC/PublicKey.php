...
use OCA\Libresign\Vendor\phpseclib4\Crypt\EC\Formats\Keys\PKCS1;
use OCA\Libresign\Vendor\phpseclib4\Crypt\EC\Formats\Signature\ASN1 as ASN1Signature;
...
final class PublicKey extends EC implements Common\PublicKey
{
    ...
    public function verify(string $message, string|array $signature) : bool
    {
        ...
        [$x1] = $this->curve->multiplyAddPoints([$this->curve->getBasePoint(), $this->QA], [$u1, $u2]);
        $x1 = $x1->toBigInteger();
        [, $x1] = $x1->divide($order);
        return $x1->equals($r);
    }
