...
use OCA\Libresign\Vendor\phpseclib4\Crypt\Common\Formats\Keys\PKCS8 as Progenitor;
...
use OCA\Libresign\Vendor\phpseclib4\Crypt\EC\Curves\Ed448;
...
abstract class PKCS8 extends Progenitor
{
    ...
    public static function load(#[\SensitiveParameter] string $key, #[\SensitiveParameter] ?string $password = null) : array
    {
        ...
        $components['QA'] = isset($key['publicKey']) ? self::extractPoint((string) $key['publicKey'], $components['curve']) : $components['curve']->multiplyPoint($components['curve']->getBasePoint(), $components['dA']);
        return $components;
    }
    /**
    ...
    private static function loadEdDSA(array $key) : array
    {
        ...
        if (isset($key['privateKey'])) {
            ...
            if (\substr($key['privateKey'], 0, 2) != $expected) {
                throw new UnexpectedValueException('The first two bytes of the ' . $key['privateKeyAlgorithm']['algorithm'] . ' private key field should be 0x' . \bin2hex($expected));
            }
    ...
    public static function savePublicKey(BaseCurve $curve, array $publicKey, array $options = []) : string
    {
        ...
        if ($curve instanceof MontgomeryCurve) {
            return self::wrapPublicKey(key: \str_pad(\strrev($publicKey[0]->toBytes()), $curve::SIZE, "\x00", \STR_PAD_RIGHT), oid: $curve instanceof Curve25519 ? 'id-X25519' : 'id-X448', options: $options);
        }
        if ($curve instanceof TwistedEdwardsCurve) {
            return self::wrapPublicKey(key: $curve->encodePoint($publicKey), oid: $curve instanceof Ed25519 ? 'id-Ed25519' : 'id-Ed448', options: $options);
        }
        $params = new ASN1\Element(self::encodeParameters($curve, \false, $options));
        $key = "\x04" . $publicKey[0]->toBytes() . $publicKey[1]->toBytes();
        return self::wrapPublicKey(key: $key, params: $params, oid: 'id-ecPublicKey', options: $options);
    }
    /**
    ...
    public static function savePrivateKey(#[\SensitiveParameter] BigInteger $privateKey, BaseCurve $curve, array $publicKey, #[\SensitiveParameter] ?string $secret = null, #[\SensitiveParameter] ?string $password = null, array $options = []) : string
    {
        self::initialize_static_variables();
        if ($curve instanceof MontgomeryCurve) {
            return self::wrapPrivateKey(key: \chr(ASN1::TYPE_OCTET_STRING) . ASN1::encodeLength($curve::SIZE) . \str_pad($privateKey->toBytes(), $curve::SIZE, "\x00", \STR_PAD_LEFT), password: $password, oid: $curve instanceof Curve25519 ? 'id-X25519' : 'id-X448');
        }
        if ($curve instanceof TwistedEdwardsCurve) {
            return self::wrapPrivateKey(key: "\x04" . \chr(\strlen($secret)) . $secret, password: $password, oid: $curve instanceof Ed25519 ? 'id-Ed25519' : 'id-Ed448', options: $options);
        }
        ...
        return self::wrapPrivateKey(key: $key, params: $params, password: $password, oid: 'id-ecPublicKey', options: $options);
    }
}
