...
use OCA\Libresign\Vendor\phpseclib4\Crypt\EC\BaseCurves\Prime as PrimeCurve;
...
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\OIDs\Curves;
...
trait Common
{
     ...
     * Use Named Curves
     */
    private static bool $useNamedCurves = \true;
    ...
    protected static function loadCurveByParam(array $params) : BaseCurve
    {
        ...
        if (isset($params['namedCurve'])) {
            $curve = '\\OCA\\Libresign\\Vendor\\phpseclib4\\Crypt\\EC\\Curves\\' . $params['namedCurve'];
        ...
        if (isset($params['specifiedCurve'])) {
            ...
            switch ($data['fieldID']['fieldType']) {
                case 'prime-field':
                    ...
                    $curve->setCoefficients(new BigInteger((string) $data['curve']['a'], 256), new BigInteger((string) $data['curve']['b'], 256));
                ...
                case 'characteristic-two-field':
                    ...
                    //$len = ceil($modulo[0] / 8);
                    $curve->setCoefficients(Strings::bin2hex((string) $data['curve']['a']), Strings::bin2hex((string) $data['curve']['b']));
                    $point = self::extractPoint("\x00" . $data['base'], $curve);
                    $curve->setBasePoint(...$point);
                    $curve->setOrder($data['order']);
                    return $curve;
    ...
    private static function encodeParameters(BaseCurve $curve, bool $returnArray = \false, array $options = []) : string|array
    {
        ...
        if ($useNamedCurves) {
            ...
            foreach (new \DirectoryIterator(__DIR__ . '/../../Curves/') as $file) {
                ...
                $class = 'OCA\\Libresign\\Vendor\\phpseclib4\\Crypt\\EC\\Curves\\' . $testName;
