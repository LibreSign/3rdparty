...
use OCA\Libresign\Vendor\phpseclib4\Crypt\Common\Formats\Keys\PKCS1 as Progenitor;
...
abstract class PKCS1 extends Progenitor
{
    ...
    public static function load(#[\SensitiveParameter] string $key, #[\SensitiveParameter] ?string $password = null) : array
    {
        ...
        if (\str_contains($key, 'BEGIN EC PARAMETERS') && \str_contains($key, 'BEGIN EC PRIVATE KEY')) {
            ...
            $components['QA'] = isset($ecPrivate['publicKey']) ? self::extractPoint((string) $ecPrivate['publicKey'], $components['curve']) : $components['curve']->multiplyPoint($components['curve']->getBasePoint(), $components['dA']);
            return $components;
        }
