...
use OCA\Libresign\Vendor\phpseclib4\Crypt\Common\StreamCipher;
...
class Salsa20 extends StreamCipher
{
    ...
    protected function poly1305(string $text) : string
    {
        if (!$this->usingGeneratedPoly1305Key) {
            ...
            instead of 96-bits
            */
            return parent::poly1305(self::nullPad128($this->aad) . self::nullPad128($text) . \pack('V', \strlen($this->aad)) . "\x00\x00\x00\x00" . \pack('V', \strlen($text)) . "\x00\x00\x00\x00");
        }
    }
}
