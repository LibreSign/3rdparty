...
abstract class PuTTY
{
     ...
     */
    private static function generateV3Key(#[\SensitiveParameter] string $password, string $flavour, int $memory, int $passes, string $salt) : array
    {
     ...
     */
    public static function load(#[\SensitiveParameter] string $key, #[\SensitiveParameter] ?string $password) : array
    {
        ...
        $publicLength = (int) \trim(\preg_replace('#Public-Lines: (\\d+)#', '$1', $key[3]));
            ...
            case 'aes256-cbc':
                ...
                switch ($version) {
                    case 3:
                        ...
                        ['symkey' => $symkey, 'symiv' => $symiv, 'hashkey' => $hashkey] = self::generateV3Key($password, $flavour, (int) $memory, (int) $passes, $salt);
                        break;
                    case 2:
        ...
        $privateLength = (int) \trim(\preg_replace('#Private-Lines: (\\d+)#', '$1', $key[$offset++]));
     ...
     */
    protected static function wrapPrivateKey(string $public, #[\SensitiveParameter] string $private, string $type, #[\SensitiveParameter] ?string $password, array $options = []) : string
    {
        ...
        if (!isset($password)) {
            ...
            switch ($version) {
                case 3:
                    ...
                    ['symkey' => $symkey, 'symiv' => $symiv, 'hashkey' => $hashkey] = self::generateV3Key($password, 'Argon2id', 8192, 13, $salt);
                    $hash = new Hash('sha256');
                    $hash->setKey($hashkey);
                    break;
                case 2:
