...
abstract class PKCS8 extends PKCS
{
    ...
    private static function initialize_static_variables() : void
    {
        if (!isset(self::$childOIDsLoaded[static::class])) {
            if (static::OID_NAME !== '') {
                ASN1::loadOIDs(\is_array(static::OID_NAME) ? \array_combine(static::OID_NAME, static::OID_VALUE) : [(string) static::OID_NAME => static::OID_VALUE]);
            }
            self::$childOIDsLoaded[static::class] = \true;
        }
        if (!self::$oidsLoaded) {
            ASN1::loadOIDs('PKCS8');
            self::$oidsLoaded = \true;
     ...
     */
    protected static function load(#[\SensitiveParameter] string $key, #[\SensitiveParameter] ?string $password = null) : array
    {
     ...
     */
    protected static function wrapPrivateKey(#[\SensitiveParameter] string $key, Element|BaseType $params = new ExplicitNull(), #[\SensitiveParameter] ?string $password = null, ?string $oid = null, array $options = []) : string
    {
        ...
        if (isset($password)) {
            ...
            $key = ['encryptionAlgorithm' => $crypto->getMetaData('algorithmIdentifier'), 'encryptedData' => $key];
