...
namespace OCA\Libresign\Vendor\phpseclib4\Crypt\Common\Traits;

use OCA\Libresign\Vendor\phpseclib4\Crypt\AES;
use OCA\Libresign\Vendor\phpseclib4\Crypt\DES;
use OCA\Libresign\Vendor\phpseclib4\Crypt\RC2;
...
trait ASN1AlgorithmIdentifier
{
    ...
    private static function getCryptoObjectFromAlgorithmIdentifier(array|Constructed $data, #[\SensitiveParameter] string $password) : SymmetricKey
    {
        $meta = [];
    ...
    protected static function setupPBKDF2(array|Constructed $keyDerivationFunc, #[\SensitiveParameter] string $password, SymmetricKey $cipher) : void
    {
        switch ($keyDerivationFunc['algorithm']) {
            case 'id-PBKDF2':
                ...
                $params = [$password, 'pbkdf2', $hash, (string) $salt, (int) $iterationCount->toString()];
                if (isset($keyLength)) {
    ...
    private static function getCryptoObjectFromParams(string $password, array $options) : SymmetricKey
    {
        ...
        $crypto->setMetaData('algorithmIdentifier', ['algorithm' => $encryptionAlgorithm, 'parameters' => new ASN1\Element($params)]);
