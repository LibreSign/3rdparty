...
use OCA\Libresign\Vendor\phpseclib4\Math\BinaryField;
use OCA\Libresign\Vendor\phpseclib4\Math\PrimeField;
/**
 * Base Class for all \phpseclib4\Crypt\* cipher classes
 *
 * @author  Jim Wigginton <terrafrost@php.net>
...
abstract class SymmetricKey
{
     ...
     */
    public const MODE_MAP = ['ctr' => self::MODE_CTR, 'ecb' => self::MODE_ECB, 'cbc' => self::MODE_CBC, 'cfb' => self::MODE_CFB, 'cfb8' => self::MODE_CFB8, 'ofb' => self::MODE_OFB, 'ofb8' => self::MODE_OFB8, 'gcm' => self::MODE_GCM, 'stream' => self::MODE_STREAM];
    /**
     ...
     */
    public const ENGINE_MAP = [self::ENGINE_INTERNAL => 'PHP', self::ENGINE_EVAL => 'Eval', self::ENGINE_OPENSSL => 'OpenSSL', self::ENGINE_LIBSODIUM => 'libsodium', self::ENGINE_OPENSSL_AEAD => 'OpenSSL (AEAD)'];
    /**
     * The Encryption Mode
     *
     ...
     */
    protected bool $changed = \true;
    /**
     * Does Eval engie need to be (re)initialized?
     *
     * @see self::setup()
     */
    protected bool $nonIVChanged = \true;
    /**
     * Padding status
     *
     * @see self::enablePadding()
     */
    private bool $padding = \true;
    /**
     ...
     * @see Crypt/Hash.php
     */
    public function setPassword(#[\SensitiveParameter] string $password, string $method = 'pbkdf2', int|string ...$func_args) : void
    {
        $key = '';
        ...
        if ($this->engine === self::ENGINE_OPENSSL) {
            switch ($this->mode) {
                ...
                case self::MODE_CFB:
                    ...
                    if ($overflow) {
                        $ciphertext .= \openssl_encrypt(\substr($plaintext, 0, -$overflow) . \str_repeat("\x00", $this->block_size), $this->cipher_name_openssl, $this->key, \OPENSSL_RAW_DATA, $iv);
            ...
            switch ($this->mode) {
                ...
                case self::MODE_CFB:
                    ...
                    if ($overflow) {
                        ...
                        $iv = \openssl_encrypt(\str_repeat("\x00", $this->block_size), $this->cipher_name_openssl, $this->key, \OPENSSL_RAW_DATA, $iv);
    ...
    private function openssl_ctr_process(string $plaintext, string &$encryptIV, array &$buffer) : string
    {
        ...
        if ($overflow) {
            ...
            $encrypted = \openssl_encrypt($plaintext . \str_repeat("\x00", $block_size), $this->cipher_name_openssl, $key, \OPENSSL_RAW_DATA, $encryptIV);
            ...
            $ciphertext .= \openssl_encrypt($plaintext . \str_repeat("\x00", $block_size), $this->cipher_name_openssl, $key, \OPENSSL_RAW_DATA, $encryptIV);
    ...
    private function openssl_ofb_process(string $plaintext, string &$encryptIV, array &$buffer) : string
    {
        ...
        if (\strlen($plaintext)) {
            if ($overflow) {
                $ciphertext .= \openssl_encrypt(\substr($plaintext, 0, -$overflow) . \str_repeat("\x00", $block_size), $this->cipher_name_openssl, $key, \OPENSSL_RAW_DATA, $encryptIV);
    ...
    protected function isValidEngineHelper(int $engine) : bool
    {
        switch ($engine) {
            case self::ENGINE_OPENSSL:
                ...
                $result = isset($this->cipher_name_openssl) && \extension_loaded('openssl');
                if (!$result) {
                    return \false;
                }
    ...
    {
        $this->engine = 0;
        $candidateEngines = [self::ENGINE_LIBSODIUM, self::ENGINE_OPENSSL_AEAD, self::ENGINE_OPENSSL, self::ENGINE_EVAL];
     ...
     */
    protected abstract function encryptBlock(string $in) : string;
    /**
     * Decrypts a block
     *
     * Note: Must be extended by the child \phpseclib4\Crypt\* class
     */
    protected abstract function decryptBlock(string $in) : string;
    /**
     ...
     * @see self::setup()
     */
    protected abstract function setupKey() : void;
    /**
                    ...
                    if (strlen($_buffer["ciphertext"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
                            ...
                            if (strlen($_block) > strlen($_buffer["ciphertext"])) {
                                $in = $_xor;
                                ' . $encrypt_block . '
                                \\OCA\\Libresign\\Vendor\\phpseclib4\\Common\\Functions\\Strings::increment_str($_xor);
                                $_buffer["ciphertext"].= $in;
                            }
                            $_key = \\OCA\\Libresign\\Vendor\\phpseclib4\\Common\\Functions\\Strings::shift($_buffer["ciphertext"], ' . $block_size . ');
                ...
                $encrypt = $init_encrypt . '
                    ...
                    if (strlen($_buffer["xor"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
                            ...
                            $_key = \\OCA\\Libresign\\Vendor\\phpseclib4\\Common\\Functions\\Strings::shift($_buffer["xor"], ' . $block_size . ');
                            $_ciphertext.= $_block ^ $_key;
                        }
                    } else {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
...
PHP
    ...
    protected static function safe_intval(int|float $x) : int
    {
        ...
        return (int) (\fmod($x, 0x80000000) & 0x7fffffff | (\fmod(\floor($x / 0x80000000), 2) & 1) << 31);
    }
    /**
    ...
    {
        $len = \strlen($str);
        return $str . \str_repeat("\x00", 16 * (int) \ceil($len / 16) - $len);
    }
    /**
