...
class Hash
{
    ...
    private function computeKey() : void
    {
        ...
        $this->computedKey = \is_array($this->algo) ? ($this->algo)($this->key) : \hash($this->algo, $this->key, \true);
    }
    /**
    ...
    public function setHash(string $hash) : void
    {
        ...
        if (\in_array(\substr($hash, 0, 5), ['shake', 'kecca'])) {
            ...
            $hash = ['OCA\\Libresign\\Vendor\\phpseclib4\\Crypt\\Hash', \PHP_INT_SIZE == 8 ? 'sha3_64' : 'sha3_32'];
        }
        if (\is_array($hash)) {
            $b = $this->blockSize >> 3;
    ...
    public function hash(mixed $text) : string
    {
        ...
        if ($algo == 'umac') {
            ...
            if ($this->recomputeAESKey) {
                ...
                if (!isset(self::$maxwordrange64)) {
                    ...
                    $temp = new BigInteger("\x01\x00\x00\x00\x00\x00\x00\x00\x00", 256);
                    self::$offset64 = self::$factory64->newInteger($temp->subtract($prime64));
                    $temp = new BigInteger("\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 256);
        ...
        if (\is_resource($text)) {
            $pos = \ftell($text);
            \rewind($text);
            $ctx = !empty($this->key) || \is_string($this->key) ? \hash_init($algo, \HASH_HMAC, $this->computedKey) : \hash_init($algo);
            \hash_update_stream($ctx, $text);
            \fseek($text, $pos);
            $output = \hash_final($ctx, \true);
        } else {
    ...
    public function setPassword(#[\SensitiveParameter] string $password, string $salt, int $iterationCount) : void
    {
