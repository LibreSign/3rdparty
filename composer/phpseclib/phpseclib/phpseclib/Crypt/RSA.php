...
use OCA\Libresign\Vendor\phpseclib4\Crypt\Common\AsymmetricKey;
use OCA\Libresign\Vendor\phpseclib4\Crypt\RSA\Formats\Keys\PSS;
use OCA\Libresign\Vendor\phpseclib4\Crypt\RSA\PrivateKey;
use OCA\Libresign\Vendor\phpseclib4\Crypt\RSA\PublicKey;
use OCA\Libresign\Vendor\phpseclib4\Exception\BadConfigurationException;
use OCA\Libresign\Vendor\phpseclib4\Exception\BadMethodCallException;
...
abstract class RSA extends AsymmetricKey
{
     ...
     * Enable Blinding?
     */
    protected static bool $enableBlinding = \true;
    /**
     * Enable automatic salt length determination
     */
    protected static bool $autoSaltLength = \true;
    /**
     * Smallest Prime
     *
    ...
    protected function emsa_pkcs1_v1_5_encode(string $m, int $emLen) : string
    {
        ...
        $t = match ($this->hash->getHash()) {
            'md2' => "0 0\f\x06\x08*\x86H\x86\xf7\r\x02\x02\x05\x00\x04\x10",
            'md5' => "0 0\f\x06\x08*\x86H\x86\xf7\r\x02\x05\x05\x00\x04\x10",
            ...
            'sha224' => "0-0\r\x06\t`\x86H\x01e\x03\x04\x02\x04\x05\x00\x04\x1c",
            'sha512/224' => "0-0\r\x06\t`\x86H\x01e\x03\x04\x02\x05\x05\x00\x04\x1c",
            'sha512/256' => "010\r\x06\t`\x86H\x01e\x03\x04\x02\x06\x05\x00\x04 ",
            ...
            'sha3/224' => "0-0\r\x06\t`\x86H\x01e\x03\x04\x02\x07\x05\x00\x04\x1c",
            'sha3/256' => "0-0\r\x06\t`\x86H\x01e\x03\x04\x02\x08\x05\x00\x04 ",
            'sha3/384' => "0-0\r\x06\t`\x86H\x01e\x03\x04\x02\t\x05\x00\x040",
            'sha3/512' => "0-0\r\x06\t`\x86H\x01e\x03\x04\x02\n\x05\x00\x04@",
        };
        $t .= $h;
        $tLen = \strlen($t);
        if ($emLen < $tLen + 11) {
    ...
    protected function emsa_pkcs1_v1_5_encode_without_null(string $m, int $emLen) : string
    {
        ...
        // see http://tools.ietf.org/html/rfc3447#page-43
        $t = match ($hashName) {
            'sha1' => "0\x1f0\x07\x06\x05+\x0e\x03\x02\x1a\x04\x14",
            'sha256' => "0/0\v\x06\t`\x86H\x01e\x03\x04\x02\x01\x04 ",
            'sha384' => "0?0\v\x06\t`\x86H\x01e\x03\x04\x02\x02\x040",
            'sha512' => "0O0\v\x06\t`\x86H\x01e\x03\x04\x02\x03\x04@",
            // from https://www.emc.com/collateral/white-papers/h11300-pkcs-1v2-2-rsa-cryptography-standard-wp.pdf#page=40
            'sha224' => "0+0\v\x06\t`\x86H\x01e\x03\x04\x02\x04\x04\x1c",
            'sha512/224' => "0+0\v\x06\t`\x86H\x01e\x03\x04\x02\x05\x04\x1c",
            'sha512/256' => "0/0\v\x06\t`\x86H\x01e\x03\x04\x02\x06\x04 ",
            // the following 3x algorithms are not specified in PKCS1 v2.2, however, some standards none-the-less do use them:
            // https://sk-eid.github.io/smart-id-documentation/rp-api/changes.html#_security_enhancements
            // the OIDs are from this URL:
            // https://csrc.nist.gov/projects/computer-security-objects-register/algorithm-registration#Hash
            'sha3/224' => "0+0\v\x06\t`\x86H\x01e\x03\x04\x02\x07\x04\x1c",
            'sha3/256' => "0+0\v\x06\t`\x86H\x01e\x03\x04\x02\x08\x04 ",
            'sha3/384' => "0+0\v\x06\t`\x86H\x01e\x03\x04\x02\t\x040",
            'sha3/512' => "0+0\v\x06\t`\x86H\x01e\x03\x04\x02\n\x04@",
        };
        $t .= $h;
        $tLen = \strlen($t);
        if ($emLen < $tLen + 11) {
    ...
    public function withHash(string $hash) : static
    {
        ...
        switch (\strtolower($hash)) {
            ...
            case 'sha3/512':
                ...
                break;
            default:
                throw new UnsupportedAlgorithmException("The only supported hash algorithms are: md2, md5, sha1, sha256, sha384, sha512, sha224, sha512/224, sha512/256, sha3/224, sha3/256, sha3/384, sha3/512 - {$hash} provided");
        }
    ...
    public function withPadding(int $padding) : static
    {
        ...
        $masks = [self::SIGNATURE_PSS, self::SIGNATURE_PKCS1];
        $signatureCount = 0;
        $selected = 0;
        foreach ($masks as $mask) {
            if ($padding & $mask) {
     ...
     */
    protected function handleOpenSSL(#[\SensitiveParameter] string $func, string $message, ?string $signature = null) : bool|null|string
    {
        ...
        if (self::$forcedEngine !== 'PHP') {
            ...
            if ($this->{$paddingType} === self::SIGNATURE_PSS) {
                ...
                $error = match (\true) {
                    !\defined('OPENSSL_PKCS1_PSS_PADDING') => 'Engine OpenSSL is forced but PSS encryption requires PHP >= 8.5.0',
                    $this->hash->getHash() !== $this->mgfHash->getHash() => 'Engine OpenSSL is forced but can\'t be used because the Hash and MGF Hash do not match',
                    $this->getSaltLength() !== $this->hLen => 'Engine OpenSSL is forced but can\'t be used because the salt length doesn\'t match the hash length',
                    !$create && !static::$autoSaltLength => 'Engine OpenSSL is forced but auto calculation of the salt length is disabled',
                    $create && $this->getLength() < 8 * (2 * $this->getSaltLength() + 2) => 'Engine OpenSSL is forced but can\'t be used for PSS signing because the key is too small for OpenSSL to use the configured salt length',
                    $create && \OPENSSL_VERSION_NUMBER < 0x30100000 => 'Engine OpenSSL is forced but can\'t be used for PSS signing because OpenSSL < 3.1.0 defaults to the maximum salt length instead of the hash length',
                    default => null,
                };
            }
            /*
            ...
            */
            ...
            if ($this->{$paddingType} === self::ENCRYPTION_OAEP) {
                $error = match (\true) {
                    $this->hash->getHash() !== $this->mgfHash->getHash() => 'Engine OpenSSL is forced but can\'t be used because the Hash and MGF Hash do not match',
                    $this->hash->getHash() !== 'sha1' && \PHP_VERSION_ID < 80500 => 'Engine OpenSSL is forced but non-sha1 hashes are only supported on PHP 8.5.0+',
                    \strlen($this->label) => 'Engine OpenSSL is forced but can\'t be used because the label is not the empty string',
                    default => null,
                };
            }
            if (isset($error)) {
    ...
    {
        if ($this instanceof PrivateKey && \count($this->primes ?? [])) {
            return ['e' => clone $this->publicExponent, 'n' => clone $this->modulus, 'd' => clone $this->exponent, 'primes' => \array_map(fn(BigInteger $var): BigInteger => clone $var, $this->primes), 'exponents' => \array_map(fn(BigInteger $var): BigInteger => clone $var, $this->exponents), 'coefficients' => \array_map(fn(BigInteger $var): BigInteger => clone $var, $this->coefficients)];
        }
        return ['e' => clone $this->publicExponent, 'n' => clone $this->modulus];
    }
}
