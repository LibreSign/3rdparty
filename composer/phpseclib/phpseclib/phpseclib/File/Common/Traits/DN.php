...
namespace OCA\Libresign\Vendor\phpseclib4\File\Common\Traits;

use OCA\Libresign\Vendor\phpseclib4\Common\Functions\Strings;
use OCA\Libresign\Vendor\phpseclib4\Crypt\Hash;
use OCA\Libresign\Vendor\phpseclib4\Exception\CharacterConversionException;
...
trait DN
{
    ...
    private static function addDNPropsInternal(array|Choice &$dn, string $propName, string|BaseString|array|Element|Constructed $value) : void
    {
        ...
        if ($propName == 'id-at-postalAddress' && \is_string($value) && \preg_match('/^#(?:[0-9A-Fa-f][0-9A-Fa-f])+$/', $value)) {
            $temp = ['type' => $propName, 'value' => new Element(\pack('H*', \substr($value, 1)))];
            self::mapInDNs($temp);
            $dn['rdnSequence'][] = [$temp];
            return;
        }
        $dn['rdnSequence'][] = [['type' => $propName, 'value' => $value]];
    }
    ...
    private static function setDNInternal(array|Choice &$dn, array|string|Element $props) : void
    {
        ...
        if ($props[0] == '/') {
            ...
            foreach ($props as $prop) {
                ...
                $temp = ['type' => self::translateDNProp($propName), 'value' => new Element($propValue)];
                self::mapInDNs($temp);
        ...
        foreach ($props as $prop) {
            ...
            if (\preg_match('/^#(?:[0-9A-Fa-f][0-9A-Fa-f])+$/', $propValue)) {
                $temp = ['type' => self::translateDNProp($propName), 'value' => new Element(\pack('H*', \substr($propValue, 1)))];
                self::mapInDNs($temp);
