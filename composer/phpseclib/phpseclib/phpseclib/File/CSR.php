...
class CSR implements \ArrayAccess, \Countable, \Iterator, Signable
{
    ...
    private Constructed|array $csr = ['certificationRequestInfo' => ['version' => 'v1', 'subject' => ['rdnSequence' => []], 'subjectPKInfo' => ['algorithm' => ['algorithm' => '0.0'], 'subjectPublicKey' => "\x00"], 'attributes' => []], 'signatureAlgorithm' => ['algorithm' => '0.0'], 'signature' => "\x00"];
    ...
    {
        $this->csr['certificationRequestInfo']['subjectPKInfo'] = ['algorithm' => ['algorithm' => '0.0'], 'subjectPublicKey' => "\x00"];
    }
    ...
    public function toString(array $options = []) : string
    {
        ...
        if ($publicKey instanceof PublicKey) {
            $origKey = $publicKey;
            $this->csr['certificationRequestInfo']['subjectPKInfo'] = new Element($publicKey->toString('PKCS8', ['binary' => \true]));
        }
        $this->mapOutDNs();
    ...
    {
        $this->csr['signature'] = new BitString("\x00{$signature}");
    }
    /**
    ...
    public function setAttribute(string $type, mixed $value) : void
    {
        ...
        $this->csr['certificationRequestInfo']['attributes'][] = ['type' => $type, 'value' => $value];
    }
    /** @psalm-suppress PossiblyUnusedMethod */
    ...
    public function setExtension(string $name, mixed $value, ?bool $critical = null) : void
    {
        ...
        $extensionRequestTemplate = ['type' => 'pkcs-9-at-extensionRequest', 'value' => [[['extnId' => $name, 'critical' => $critical, 'extnValue' => $value]]]];
        if (!isset($this->csr['certificationRequestInfo']['attributes'])) {
    ...
    {
        $value = \is_string($password) || $password instanceof UTF8String ? ['utf8String' => $password] : ['printableString' => $password];
    ...
    {
        return self::validateSignatureHelper($this->getPublicKey(), $this->csr['signatureAlgorithm']['algorithm'], $this->csr['signature'], $this->csr['certificationRequestInfo']->getEncoded());
    }
}
