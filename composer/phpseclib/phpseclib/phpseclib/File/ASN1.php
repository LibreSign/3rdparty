...
namespace OCA\Libresign\Vendor\phpseclib4\File;

use OCA\Libresign\Vendor\phpseclib4\Common\Functions\Strings;
use OCA\Libresign\Vendor\phpseclib4\Crypt\Common\PublicKey;
use OCA\Libresign\Vendor\phpseclib4\Exception\BadConfigurationException;
use OCA\Libresign\Vendor\phpseclib4\Exception\EOCException;
use OCA\Libresign\Vendor\phpseclib4\Exception\EncodedDataUnavailableException;
use OCA\Libresign\Vendor\phpseclib4\Exception\ResourceLimitException;
...
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\MalformedData;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\BMPString;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\BaseType;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\BitString;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\Boolean;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\Choice;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\ExplicitNull;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\GeneralString;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\GeneralizedTime;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\GraphicString;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\IA5String;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\Integer;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\NumericString;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\OID;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\OctetString;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\PrintableString;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\TeletexString;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\UTCTime;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\UTF8String;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\VideotexString;
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\VisibleString;
use OCA\Libresign\Vendor\phpseclib4\File\CMS\EnvelopedData\KeyAgreeRecipient\EncryptedKey;
use OCA\Libresign\Vendor\phpseclib4\File\CMS\EnvelopedData\Recipient;
use OCA\Libresign\Vendor\phpseclib4\File\CMS\SignedData\Signer;
...
abstract class ASN1
{
     ...
     * @see self::encodeDER()
     */
    private static bool $useEncodedCache = \true;
     ...
     */
    private static bool $invalidateCache = \true;
    ...
    public static function decodeBER(string $encoded, int $start = 0, int $encoded_pos = 0) : array
    {
        ...
        ['constructed' => $constructed, 'tag' => $tag, 'class' => $class] = self::decodeTag($encoded, $encoded_pos);
        ...
        switch ($class) {
            ...
            case self::CLASS_CONTEXT_SPECIFIC:
                return ['start' => $start, 'type' => $class, 'constant' => $tag, 'content' => $constructed ? new Constructed($content, $class, $tag, $start, 0, $current['headerlength'], $headercontent) : $content, 'length' => $length] + $current;
        }
        $current += ['type' => $tag];
        if ($constructed) {
            switch ($tag) {
            ...
            return $current + ['content' => new Constructed($content, $class, $tag, $start, $encoded_pos, $current['headerlength'], $headercontent)];
        }
        ...
        $current['content']->addMetadata(['rawheader' => $headercontent, 'content' => $content] + $current);
        //$start += $length;
        return $current;
    }
    /**
    ...
    public static function map(array $decoded, array $mapping, array $rules = []) : Element|BaseType
    {
        ...
        throw new UnexpectedValueException('Unable to perform mapping - found ' . self::convertTypeConstantToString($decoded['type']) . ' - expected ' . self::convertTypeConstantToString($mapping['type']));
    }
        ...
        switch ($tag) {
            ...
            case self::TYPE_ENUMERATED:
                if (!\is_string($source) && !$source instanceof BigInteger && !\is_numeric($source)) {
                    $message = \implode('/', self::$location) . ' must be a string, a numeric (is_numeric), or an instance of ' . 'either phpseclib4\\File\\ASN1\\Types\\Integer or a phpseclib4\\Math\\BigInteger';
    ...
    private static function decodeTime(string $content, int $tag) : UTCTime|GeneralizedTime
    {
        ...
        $result = $tag == self::TYPE_UTC_TIME ? UTCTime::createFromFormat($format, $content) : GeneralizedTime::createFromFormat($format, $content);
        if ($result === \false) {
    ...
    public static function loadOIDs(array|string $oids) : void
    {
        ...
        $class = 'OCA\\Libresign\\Vendor\\phpseclib4\\File\\ASN1\\OIDs\\' . $oids;
