...
namespace OCA\Libresign\Vendor\phpseclib4\File\CMS;

use OCA\Libresign\Vendor\phpseclib4\Common\Functions\Strings;
use OCA\Libresign\Vendor\phpseclib4\Crypt\AES;
...
use OCA\Libresign\Vendor\phpseclib4\File\CMS;
use OCA\Libresign\Vendor\phpseclib4\File\CRL;
use OCA\Libresign\Vendor\phpseclib4\File\X509;
use OCA\Libresign\Vendor\phpseclib4\File\CMS\EnvelopedData\DerivableKey;
use OCA\Libresign\Vendor\phpseclib4\File\CMS\EnvelopedData\KEKRecipient;
use OCA\Libresign\Vendor\phpseclib4\File\CMS\EnvelopedData\KeyAgreeRecipient;
use OCA\Libresign\Vendor\phpseclib4\File\CMS\EnvelopedData\KeyTransRecipient;
use OCA\Libresign\Vendor\phpseclib4\File\CMS\EnvelopedData\PasswordRecipient;
use OCA\Libresign\Vendor\phpseclib4\File\CMS\EnvelopedData\Recipient;
use OCA\Libresign\Vendor\phpseclib4\File\CMS\EnvelopedData\SearchableKey;
/**
 * Pure-PHP CMS / EncryptedData Parser
 *
 * @author  Jim Wigginton <terrafrost@php.net>
...
{
    use \OCA\Libresign\Vendor\phpseclib4\File\Common\Traits\KeyDerivation;
    private Constructed|array $cms;
    public string $cek;
    // content encryption key
    public function __construct(#[\SensitiveParameter] string $data, string $encryptionAlgorithm = 'aes128-CBC-PAD', #[\SensitiveParameter] ?string $key = null)
    {
        ...
        $this->cms = ['contentType' => 'id-encryptedData', 'content' => [
            ...
            'version' => 'v0',
            'encryptedContentInfo' => ['contentType' => 'id-data', 'contentEncryptionAlgorithm' => ['algorithm' => $encryptionAlgorithm, 'parameters' => new OctetString($iv)], 'encryptedContent' => $encrypted],
        ]];
    }
    ...
    public function toString(array $options = []) : string
    {
        ...
        if ($this->cms instanceof Constructed) {
            ...
            $temp = ['contentType' => $this->cms['contentType'], 'content' => new Element(ASN1::encodeDER($this->cms['content'], $map))];
    ...
    {
        if (!$this->cms instanceof Constructed) {
            $temp = self::load($this->toString(['binary' => \true]));
            $this->cms = $temp->cms;
            return;
        }
        ...
        $temp = self::load($this->toString(['binary' => \true]))->cms['content'];
    ...
    public function createNewRecipientFromPassword(#[\SensitiveParameter] string $password, string $encryptionAlgorithm = 'aes128-CBC-PAD') : PasswordRecipient
    {
        ...
        $params = ['encryptionAlgorithm' => 'id-PBES2', 'encryptionScheme' => $encryptionAlgorithm];
        ...
        $length = \max($keyCipher->getBlockLengthInBytes(), 2 * $contentCipher->getBlockLengthInBytes()) - \strlen($this->cek) - 4;
        if ($length < 0) {
    ...
    public function createNewRecipientFromKeyWithIdentifier(#[\SensitiveParameter] string $key, string $identifier, ?\DateTimeInterface $date = null) : KEKRecipient
    {
        // to decrypt with openssl cli do this:
    ...
    public function createNewRecipientFromX509(X509 $x509, int $type = CMS::ISSUER_AND_DN) : KeyTransRecipient|KeyAgreeRecipient
    {
        ...
        if ($publicKey instanceof RSA) {
            ...
            if (!($publicKey->getPadding() & RSA::ENCRYPTION_OAEP)) {
                ...
                $algorithm = ['algorithm' => 'id-RSAES-OAEP', 'parameters' => ['hashAlgorithm' => ['algorithm' => $hash], 'maskGenAlgorithm' => ['algorithm' => 'id-mgf1', 'parameters' => ['algorithm' => $mgfHash]], 'pSourceAlgorithm' => ['algorithm' => 'id-pSpecified', 'parameters' => new OctetString($publicKey->getLabel())]]];
            ...
            $secret = \OCA\Libresign\Vendor\phpseclib4\Crypt\DH::computeSecret($privateKey, $publicKey);
            $parameters = new ASN1\Element(ASN1::encodeDER(['algorithm' => 'id-aes128-wrap'], Maps\AlgorithmIdentifier::MAP));
            $sharedInfo = ['keyInfo' => $parameters, 'suppPubInfo' => \pack('N', 128)];
            ...
            $recipient = [
                'version' => 'v3',
                // always set to v3
                'originator' => ['originatorKey' => new Element($privateKey->getPublicKey()->toString('PKCS8', ['binary' => \true]))],
    ...
    public static function wrapDES(#[\SensitiveParameter] string $kek, #[\SensitiveParameter] string $cek) : string
    {
        ...
        $temp2 = $iv . $temp1;
        $temp3 = \strrev($temp2);
        $cipher->setIV("Jݢ,y\xe8!\x05");
        return $cipher->encrypt($temp3);
    }
    // from https://www.secg.org/sec1-v2.pdf#page=38
    // $keydatalen should be the bytelength
    public static function ANSIX963KDF(string $z, int $keydatalen, string $sharedinfo, \OCA\Libresign\Vendor\phpseclib4\Crypt\Hash $hash) : string
    {
        ...
        foreach ($this->cms['content']['originatorInfo']['certs'] as $cert) {
            switch (\true) {
                ...
                case isset($cert['certificate']):
                    // && $cert['certificate'] instanceof X509:
