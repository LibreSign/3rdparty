...
namespace OCA\Libresign\Vendor\phpseclib4\File\CMS\EnvelopedData\KeyAgreeRecipient;
...
use OCA\Libresign\Vendor\phpseclib4\File\CMS\EncryptedData;
...
class EncryptedKey implements DerivableKey, SearchableKey, \ArrayAccess, \Countable, \Iterator
{
    ...
    public function withKey(#[\SensitiveParameter] EC\PrivateKey $key) : self
    {
        ...
        */
        ...
        $encoded = $private->getPublicKey()->toString('PKCS8', ['binary' => \true]);
        $decoded = ASN1::decodeBER($encoded);
        ...
        $sharedInfo = ['keyInfo' => new ASN1\Element($this->recipient['keyEncryptionAlgorithm']['parameters']->getEncoded()), 'suppPubInfo' => \pack('N', $length << 3)];
        ...
        $hash = new \OCA\Libresign\Vendor\phpseclib4\Crypt\Hash($matches[1]);
