...
namespace OCA\Libresign\Vendor\phpseclib4\File\CMS\SignedData;
...
class Signer implements \ArrayAccess, \Countable, \Iterator, Signable
{
    ...
    // pretty much just for extensionMatch()
    /**
    ...
    public function matchesX509(X509 $x509) : bool
    {
        ...
        */
        ...
        if (isset($ESSCertIDv2) || isset($ESSCertID)) {
            ...
            $expectedCert = ['issuerAndSerialNumber' => ['issuer' => $expected['issuerSerial']['issuer'][0]['directoryName'], 'serialNumber' => $expected['issuerSerial']['serialNumber']]];
            $hash = new Hash($hash);
        }
        if (isset($expected)) {
    ...
    public function validateSignature(bool $caonly = \true) : bool
    {
        ...
        */
        $signatureSubject = \count($this->signer['signedAttrs']) ? \chr(ASN1::TYPE_SET | 0x20) . \substr((string) $this->signer['signedAttrs'], 1) : (string) $this->cms['content']['encapContentInfo']['eContent'];
        ...
        return self::validateSignatureHelper($signingCert->getPublicKey(), $algo, new BitString("\x00" . $this->signer['signature']), $signatureSubject);
    }
    ...
    public function copySigningX509Attributes(X509 $x509) : void
    {
        ...
        $sid = ['issuerAndSerialNumber' => ['issuer' => $x509['tbsCertificate']['issuer'], 'serialNumber' => $x509['tbsCertificate']['serialNumber']]];
        $this->signer['sid'] = $sid;
