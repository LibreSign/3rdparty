...
namespace OCA\Libresign\Vendor\phpseclib4\File\ASN1;
...
use OCA\Libresign\Vendor\phpseclib4\Exception\ExcessivelyDeepDataException;
...
class Constructed implements \ArrayAccess, \Countable, \Iterator, BaseType
{
    ...
    public function __construct(private string $encoded, private int $class, private int $tag, private int $start, private int $encoded_pos, private int $headerlength, private string $rawheader)
    {
    }
            ...
            case ASN1::TYPE_OCTET_STRING:
                ...
                foreach ($decoded as $content) {
                    if ($content['content'] instanceof Constructed) {
                        /** @var Choice */
                        $temp = ASN1::map($content, ['type' => ASN1::TYPE_CHOICE, 'children' => ['data' => ['type' => $this->tag]]]);
                        $temp->key = 0;
    ...
    private function setSeqInnerLoop(array $child, array $temp, int $tempClass, string $key) : BaseType|Element|null
    {
        ...
        if ($child['type'] != ASN1::TYPE_CHOICE) {
            ...
            $maymatch = isset($temp['constant']) && $child['constant'] == $temp['constant'] && $tempClass == ASN1::CLASS_CONTEXT_SPECIFIC;
        }
        if ($maymatch) {
            // Attempt submapping.
            try {
    ...
    public function __debugInfo() : array
    {
        if (!isset($this->mapping)) {
            ...
            $temp = ['encoded' => '...', 'class' => $this->class, 'tag' => $this->tag, 'start' => $this->start, 'encoded_pos' => $this->encoded_pos, 'headerlength' => $this->headerlength];
            return $temp;
        }
        self::decodeCurrent();
