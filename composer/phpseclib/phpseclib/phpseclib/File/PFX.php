...
use OCA\Libresign\Vendor\phpseclib4\Exception\PasswordNeededException;
...
{
    use \OCA\Libresign\Vendor\phpseclib4\Crypt\Common\Traits\ASN1AlgorithmIdentifier;
    public Constructed|array $pfx = ['version' => 'v3', 'authSafe' => ['contentType' => 'id-data', 'content' => []]];
    ...
    public static function load(#[\SensitiveParameter] string|array|Constructed $pfx, #[\SensitiveParameter] ?string $password = null) : self
    {
        $temp = new self();
        try {
    ...
    private static function loadString(#[\SensitiveParameter] string $pfx, #[\SensitiveParameter] ?string $password) : Constructed
    {
    ...
    public function setPassword(#[\SensitiveParameter] ?string $password = null) : void
    {
        ...
        if (isset($this->password)) {
            // there already is a password; simply changing the password is sufficient
            ...
            // there is no password but there will be; CertBag's need to be moved around
     ...
     */
    public function add(#[\SensitiveParameter] X509|PrivateKey $obj, string|BaseString|array|null $friendlyName = null, string|BaseString|array|null $localKeyID = null) : void
    {
        $extra = [];
        if (isset($friendlyName) || isset($localKeyID)) {
            if (isset($friendlyName)) {
                ...
                if (\count($value)) {
                    $extra['bagAttributes'][] = ['type' => 'pkcs-9-at-friendlyName', 'value' => $value];
                }
            }
            if (isset($localKeyID)) {
                ...
                if (\count($value)) {
                    $extra['bagAttributes'][] = ['type' => 'pkcs-9-at-localKeyId', 'value' => $value];
                }
            }
        }
        if ($obj instanceof PrivateKey) {
            if (isset($this->password)) {
                $obj = ['bagId' => 'PKCS8ShroudedKeyBag', 'bagValue' => $obj->withPassword($this->password)] + $extra;
            } else {
                $obj = ['bagId' => 'KeyBag', 'bagValue' => $obj->withoutPassword()] + $extra;
            }
            $this->pfx['authSafe']['content'][] = ['contentType' => 'id-data', 'content' => [$obj]];
        } else {
            $obj = ['bagId' => 'CertBag', 'bagValue' => ['certId' => 'x509Certificate', 'certValue' => $obj]] + $extra;
            $this->pfx['authSafe']['content'][] = ['contentType' => isset($this->password) ? 'id-encryptedData' : 'id-data', 'content' => [$obj]];
        }
    }
    public function toString(array $options = []) : string
    {
        foreach ($this->pfx['authSafe']['content'] as $key => $value) {
            ...
            switch ($value['contentType']) {
                case 'id-data':
                    foreach ($value['content'] as $subkey => $subvalue) {
                        ...
                        switch ($subvalue['bagId']) {
                            ...
                            case 'KeyBag':
                            case 'PKCS8ShroudedKeyBag':
                                $subvalue['bagValue'] = new Element($subvalue['bagValue']->toString('PKCS8', ['binary' => \true]));
                        }
                ...
                case 'id-encryptedData':
                    ...
                    $value['content'] = ['version' => 'v0', 'encryptedContentInfo' => ['contentType' => 'id-data', 'contentEncryptionAlgorithm' => $crypt->getMetaData('algorithmIdentifier'), 'encryptedContent' => $value['content']]];
        ...
        if ($hashAlgorithm && isset($this->password)) {
            ...
            $this->pfx['macData'] = ['mac' => ['digestAlgorithm' => ['algorithm' => 'id-' . $hashAlgorithm, 'parameters' => null], 'digest' => $mac], 'macSalt' => $salt, 'iterations' => $iterationCount];
        }
