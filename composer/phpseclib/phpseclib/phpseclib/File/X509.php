...
use OCA\Libresign\Vendor\phpseclib4\Common\Functions\Arrays;
...
use OCA\Libresign\Vendor\phpseclib4\File\ASN1\Types\BaseString;
...
use OCA\Libresign\Vendor\phpseclib4\File\Common\Signable;
...
{
    use \OCA\Libresign\Vendor\phpseclib4\File\Common\Traits\Extension;
    use \OCA\Libresign\Vendor\phpseclib4\File\Common\Traits\DN;
    use \OCA\Libresign\Vendor\phpseclib4\File\Common\Traits\ASN1Signature;
    /**
    ...
    private static bool $strictDNComparison = \true;
    public ?self $issuer = null;
    private static \DateTimeInterface|string|null $targetValidationDate = 'now';
    private static bool $checkKeyUsage = \true;
    private static bool $checkBasicConstraints = \true;
    private int $caSeq;
    ...
    public function __construct(PublicKey|CSR|null $cert = null)
    {
        ...
        */
        $serialNumber = new BigInteger(\random_bytes(20) & "" . \str_repeat("\xff", 19), 256);
        $this->cert = ['tbsCertificate' => ['version' => 'v3', 'serialNumber' => $serialNumber, 'signature' => ['algorithm' => '0.0'], 'issuer' => ['rdnSequence' => []], 'validity' => ['notBefore' => ['utcTime' => $startDate->format('Y-m-d H:i:s')], 'notAfter' => ['utcTime' => $endDate->format('Y-m-d H:i:s')]], 'subject' => ['rdnSequence' => []], 'subjectPublicKeyInfo' => ['algorithm' => ['algorithm' => '0.0'], 'subjectPublicKey' => "\x00"]], 'signatureAlgorithm' => ['algorithm' => '0.0'], 'signature' => "\x00"];
    ...
    {
        self::$checkBasicConstraints = \true;
    }
    ...
    {
        $this->cert['tbsCertificate']['subjectPublicKeyInfo'] = ['algorithm' => ['algorithm' => '0.0'], 'subjectPublicKey' => "\x00"];
    }
    /**
    ...
    public function setSerialNumber(string|BigInteger $serial, int $base = 256) : void
    {
        ...
        if ($serial->isNegative()) {
            $serial = new BigInteger($serial->toBytes(\true), 256);
        }
    ...
    public function toString(array $options = []) : string
    {
        ...
        if ($publicKey instanceof PublicKey) {
            ...
            $this->cert['tbsCertificate']['subjectPublicKeyInfo'] = new Element($publicKey->toString($format, ['binary' => \true]));
        }
    ...
    public function createSubjectKeyIdentifier(int $method = 1) : string
    {
        ...
        $key = $this->cert['tbsCertificate']['subjectPublicKeyInfo']->toString('PKCS8', ['binary' => \true]);
        $hash = new Hash('sha1');
        $hash = $hash->hash($key);
        if ($method == 2) {
            $hash = \substr($hash, -8);
    ...
    public function getExtension(string $name) : ?array
    {
        ...
        foreach ($this->cert['tbsCertificate']['extensions'] as $ext) {
            if (self::extensionMatch($name, $ext['extnId'])) {
                return ['extnId' => $name, 'extnValue' => $ext['extnValue'], 'critical' => \is_bool($ext['critical']) ? $ext['critical'] : $ext['critical']->value];
            }
        }
        return null;
    }
    ...
    public function setExtension(string $name, mixed $value, ?bool $critical = null) : void
    {
        ...
        $this->cert['tbsCertificate']['extensions'][] = ['extnId' => $name, 'critical' => $critical, 'extnValue' => $value];
    }
    ...
    public function makeCA() : void
    {
        ...
        $this->setExtension('id-ce-keyUsage', \array_values(\array_unique(\array_merge($keyUsage, ['cRLSign', 'keyCertSign', 'digitalSignature']))));
    ...
    public static function registerExtension(string $id, array $mapping) : void
    {
        if (\is_array(self::getMapping($id))) {
            throw new InvalidStateException("Extension {$id} has already been defined with a different mapping.");
        }
    ...
    {
        $this->cert['signature'] = new BitString("\x00{$signature}");
    }
    /**
     ...
     */
    public function validateSignature(bool $caonly = \true) : bool
    {
    ...
    private function validateSignatureCountable(bool $caonly, int $count) : bool
    {
        ...
        if (!isset($signingCert)) {
            if ($caonly) {
                return $this->testForIntermediate(\true, $count) && $this->validateSignature(\true);
            } else {
                try {
                    $this->testForSelfSigned();
                    $signingCert = $this;
        ...
        $signatureResult = self::validateSignatureHelper($signingCert->getPublicKey(), $this->cert['signatureAlgorithm']['algorithm'], $this->cert['signature'], $this->cert['tbsCertificate']->getEncoded());
        $dateResult = isset(self::$targetValidationDate) ? $this->validateDate(self::$targetValidationDate) : \true;
