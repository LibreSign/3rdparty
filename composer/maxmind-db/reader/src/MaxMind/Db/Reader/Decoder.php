...
namespace OCA\Libresign\Vendor\MaxMind\Db\Reader;
...
class Decoder
{
     ...
     * @return array<mixed>
     */
    private function decodeWithBudget(int $offset, int $depth, bool $allowPointer = \true) : array
    {
        ...
        if ($type === self::_POINTER) {
            ...
            if ($depth >= self::MAX_DEPTH) {
                throw new InvalidDatabaseException('The MaxMind DB file exceeds the maximum depth');
            }
        ...
        switch ($type) {
            ...
            case self::_UTF8_STRING:
                ...
                if ($size > $this->byteBudget) {
                    throw new InvalidDatabaseException('The MaxMind DB file exceeds the maximum payload size');
                }
     ...
     */
    private function enterContainer(int $size, int $depth, int $valuesPerEntry = 1) : void
    {
        if ($depth >= self::MAX_DEPTH) {
            throw new InvalidDatabaseException('The MaxMind DB file exceeds the maximum depth');
        }
        if ($size > \intdiv($this->budget, $valuesPerEntry)) {
            throw new InvalidDatabaseException('The MaxMind DB file exceeds the maximum number of values');
        }
