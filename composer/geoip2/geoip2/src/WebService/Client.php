...
namespace OCA\Libresign\Vendor\GeoIp2\WebService;

use OCA\Libresign\Vendor\GeoIp2\Exception\AddressNotFoundException;
use OCA\Libresign\Vendor\GeoIp2\Exception\AuthenticationException;
use OCA\Libresign\Vendor\GeoIp2\Exception\GeoIp2Exception;
use OCA\Libresign\Vendor\GeoIp2\Exception\HttpException;
use OCA\Libresign\Vendor\GeoIp2\Exception\InvalidRequestException;
use OCA\Libresign\Vendor\GeoIp2\Exception\OutOfQueriesException;
use OCA\Libresign\Vendor\GeoIp2\Model\City;
use OCA\Libresign\Vendor\GeoIp2\Model\Country;
use OCA\Libresign\Vendor\GeoIp2\Model\Insights;
use OCA\Libresign\Vendor\GeoIp2\ProviderInterface;
use OCA\Libresign\Vendor\MaxMind\Exception\InsufficientFundsException;
use OCA\Libresign\Vendor\MaxMind\Exception\IpAddressNotFoundException;
use OCA\Libresign\Vendor\MaxMind\Exception\WebServiceException;
use OCA\Libresign\Vendor\MaxMind\WebService\Client as WsClient;
/**
...
class Client implements ProviderInterface
{
    ...
    private function responseFor(string $endpoint, string $class, string $ipAddress) : City|Country|Insights
    {
        ...
        } catch (\OCA\Libresign\Vendor\MaxMind\Exception\AuthenticationException $ex) {
        ...
        } catch (\OCA\Libresign\Vendor\MaxMind\Exception\InvalidRequestException $ex) {
            throw new InvalidRequestException($ex->getMessage(), $ex->getErrorCode(), $ex->getStatusCode(), $ex->getUri(), $ex);
        } catch (\OCA\Libresign\Vendor\MaxMind\Exception\HttpException $ex) {
