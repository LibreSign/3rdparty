...
use OCA\Libresign\Vendor\GeoIp2\Record\Continent;
use OCA\Libresign\Vendor\GeoIp2\Record\Country as CountryRecord;
use OCA\Libresign\Vendor\GeoIp2\Record\MaxMind;
use OCA\Libresign\Vendor\GeoIp2\Record\RepresentedCountry;
use OCA\Libresign\Vendor\GeoIp2\Record\Traits;
/**
...
class Country implements \JsonSerializable
{
    ...
    {
        $this->continent = new Continent($raw['continent'] ?? [], $locales);
        $this->country = new CountryRecord($raw['country'] ?? [], $locales);
        $this->maxmind = new MaxMind($raw['maxmind'] ?? []);
        $this->registeredCountry = new CountryRecord($raw['registered_country'] ?? [], $locales);
        $this->representedCountry = new RepresentedCountry($raw['represented_country'] ?? [], $locales);
