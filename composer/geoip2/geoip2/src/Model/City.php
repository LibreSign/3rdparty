...
use OCA\Libresign\Vendor\GeoIp2\Record\City as CityRecord;
use OCA\Libresign\Vendor\GeoIp2\Record\Location;
use OCA\Libresign\Vendor\GeoIp2\Record\Postal;
use OCA\Libresign\Vendor\GeoIp2\Record\Subdivision;
/**
...
class City extends Country
{
    ...
    public function __construct(array $raw, array $locales = ['en'])
    {
        ...
        if (empty($raw['subdivisions'])) {
            $this->subdivisions = [];
            $this->mostSpecificSubdivision = new Subdivision([], $locales);
            return;
        }
        $subdivisions = [];
        foreach ($raw['subdivisions'] as $sub) {
            $subdivisions[] = new Subdivision($sub, $locales);
        }
        // Not using end as we don't want to modify internal pointer.
        $this->mostSpecificSubdivision = $subdivisions[\count($subdivisions) - 1];
