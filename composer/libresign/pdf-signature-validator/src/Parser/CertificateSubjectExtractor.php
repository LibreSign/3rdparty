...
final class CertificateSubjectExtractor
{
    ...
    private const ATTRIBUTES = ['2.5.4.3' => 'commonName', '2.5.4.6' => 'countryName', '2.5.4.10' => 'organizationName', '1.2.840.113549.1.9.1' => 'emailAddress'];
    public function __construct(private readonly Asn1NodeReader $reader)
    {
    }
    /**
    ...
    public function extract(array $nodes) : array
    {
        ...
        foreach ($this->reader->walk($nodes) as $node) {
            ...
            if ($this->reader->type($node) === ASN1::TYPE_OBJECT_IDENTIFIER && $oid !== null && isset(self::ATTRIBUTES[$oid])) {
    ...
    {
        return \mb_check_encoding($value, 'UTF-8') && \preg_match('/[\\P{C}]/u', $value) === 1 && \preg_match('/[\\x00-\\x08\\x0B\\x0C\\x0E-\\x1F\\x7F]/', $value) !== 1;
    }
}
