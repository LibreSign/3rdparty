...
final class PdfDocumentModificationAnalyzer
{
    ...
    public function enrichLastSignatureMetadata(array $signatures, string $content) : array
    {
        ...
        $state = $this->detectDocumentModificationState($signatures[$lastSignatureIndex]->metadata, $content);
        $enrichedSignatures = [];
        foreach ($signatures as $index => $signature) {
            $metadata = $signature->metadata;
            $enrichedSignatures[] = new ExtractedSignature($signature->binarySignature, new SignatureMetadata($metadata->field, $metadata->range, $metadata->signatureType, $metadata->coversEntireDocument, $metadata->contentsOffset, $index === $lastSignatureIndex ? $state : null), $signature->hashAlgorithm);
        }
    ...
    private function detectDocumentModificationState(SignatureMetadata $metadata, string $content) : DocumentModificationState
    {
        ...
        $afterFinalEof = \substr($content, $lastEofOffset + \strlen('%%EOF'));
     ...
     */
    private function isValidByteRange(?array $range, ?int $contentsOffset, string $content) : bool
    {
        if ($range === null) {
            return \false;
        }
        return $this->hasValidRangeBounds($range, \strlen($content)) && $this->matchesContentsGap($range, $contentsOffset, $content);
    }
    /**
    ...
    {
        return $range['length1'] < 0 || $range['offset2'] < 0 || $range['length2'] < 0;
    }
    /**
     * @param array{offset1:int,length1:int,offset2:int,length2:int} $range
     */
    private function matchesContentsGap(array $range, ?int $contentsOffset, string $content) : bool
    {
        ...
        $closingDelimiterOffset = \strpos($content, '>', $contentsOffset);
        if ($closingDelimiterOffset === \false) {
            return \false;
        }
        return $range['length1'] === $openingDelimiterOffset && $range['offset2'] === $closingDelimiterOffset + 1;
    }
    private function endsAtSignedEofBoundary(string $content, int $signedEnd) : bool
    {
        $signedRevision = \substr($content, 0, $signedEnd);
        return \preg_match('/%%EOF(?:\\r\\n|\\r|\\n)?\\z/D', $signedRevision) === 1;
    }
