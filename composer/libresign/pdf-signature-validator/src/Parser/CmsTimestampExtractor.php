...
final class CmsTimestampExtractor
{
    ...
    public function __construct(private readonly Asn1NodeReader $reader = new Asn1NodeReader(), private readonly CertificateSubjectExtractor $certificateSubjectExtractor = new CertificateSubjectExtractor(new Asn1NodeReader()), private readonly TstInfoParser $tstInfoParser = new TstInfoParser(new Asn1NodeReader()))
    {
        ASN1::loadOIDs('CMS');
    }
    ...
    private function findTimestampTokenContent(array $nodes) : ?string
    {
        ...
        foreach ($this->reader->walk($nodes) as $node) {
            if ($this->reader->type($node) === ASN1::TYPE_OBJECT_IDENTIFIER && $this->reader->matchesOid($this->reader->content($node), self::TIMESTAMP_TOKEN_OID)) {
                $foundTokenOid = \true;
                continue;
            }
    ...
    private function findContentAfterOid(array $nodes, string $oid, int $expectedType) : ?string
    {
        $foundOid = \false;
        foreach ($this->reader->walk($nodes) as $node) {
            if ($this->reader->type($node) === ASN1::TYPE_OBJECT_IDENTIFIER && $this->reader->matchesOid($this->reader->content($node), $oid)) {
                $foundOid = \true;
                continue;
            }
