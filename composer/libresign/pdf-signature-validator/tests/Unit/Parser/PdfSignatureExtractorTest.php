...
final class PdfSignatureExtractorTest extends TestCase
{
    ...
    {
        $extractor = new PdfSignatureExtractor();
        $pdf = "%PDF-1.6\n" . "1 0 obj\n" . "<< /Type /Sig" . " /SubFilter /adbe.pkcs7.detached" . " /ByteRange [0 10 20 30]" . " /T (Signature1)" . " /Contents <AB CD\nEF> >>\n" . "endobj\n" . \str_repeat('X', 400);
        $result = $extractor->extractFromString($pdf);
        $this->assertCount(1, $result);
        $this->assertSame("\xab\xcd\xef", $result[0]->binarySignature);
        $this->assertSame(\strpos($pdf, "AB CD\nEF"), $result[0]->metadata->contentsOffset);
    }
    public function testKeepsDistinctSignatureDictionariesWithSameContents() : void
    {
        ...
        $this->assertLessThan($result[1]->metadata->contentsOffset, $result[0]->metadata->contentsOffset);
    }
    public function testDecodesOddLengthHexadecimalContents() : void
    {
        $extractor = new PdfSignatureExtractor();
        $pdf = $this->buildSignedPdfFixture('ABC', '/adbe.pkcs7.detached', 'Signature1', [0, 10, 20, 30]);
        $result = $extractor->extractFromString($pdf);
        $this->assertCount(1, $result);
    ...
    {
        $extractor = new PdfSignatureExtractor();
        $pdf = "%PDF-1.6\n" . "1 0 obj\n" . "<< /Type /Sig" . " /SubFilter /adbe.pkcs7.detached" . " /ByteRange [+0 +10 +20 +30]" . " /T (Signature1)" . " /Contents <ABCD> >>\n" . "endobj\n" . \str_repeat('X', 400);
        $result = $extractor->extractFromString($pdf);
        $this->assertSame(['offset1' => 0, 'length1' => 10, 'offset2' => 20, 'length2' => 50], $result[0]->metadata->range);
    }
    ...
    {
        $extractor = new PdfSignatureExtractor();
        $pdf = "%PDF-1.6\n" . "1 0 obj\n" . "<< /Type /Annot" . " /Subtype /Text" . " /Contents <FEFF00480065006C006C006F> >>\n" . "endobj\n" . "%%EOF\n";
        $this->expectException(UnsignedPdfException::class);
    ...
    {
        $extractor = new PdfSignatureExtractor();
        $pdf = $this->buildSignedPdfFixture('ABCD', '/adbe.pkcs7.detached', 'Signature1', [0, 10, 20, 999999]);
        $result = $extractor->extractFromString($pdf);
        $this->assertFalse($result[0]->metadata->coversEntireDocument);
    }
