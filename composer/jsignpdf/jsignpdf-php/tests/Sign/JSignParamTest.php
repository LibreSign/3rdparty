...
class JSignParamTest extends TestCase
{
    ...
    public function testKeepsAValueThatLooksLikeAPasswordOptionInTheParameters(string $value) : void
    {
        ...
        $this->assertSame(\escapeshellarg('--l2-text') . ' ' . \escapeshellarg($value), $params->getJSignParameters());
    }
    public static function providerValuesThatLookLikeAPasswordOption() : array
    {
        return ['short option' => ['-tsp'], 'long option' => ['--tsa-password'], 'short option with assignment' => ['-tsp=some-text'], 'long option with assignment' => ['--tsa-password=some-text']];
    }
    public function testKeepsTheFlagThatFollowsAValueLookingLikeAPasswordOption() : void
    {
        ...
        $this->assertSame([], $params->getPasswords());
        $this->assertSame(\escapeshellarg('--l2-text') . ' ' . \escapeshellarg('-tsp') . ' ' . \escapeshellarg('-V'), $params->getJSignParameters());
    }
    public function testRendersFlagsAndOptionsWithAValue() : void
    {
        $params = JSignParam::instance()->setJSignParameters(['-a', '-kst' => 'PKCS12']);
        $this->assertSame(\escapeshellarg('-a') . ' ' . \escapeshellarg('-kst') . ' ' . \escapeshellarg('PKCS12'), $params->getJSignParameters());
    }
    ...
    {
        $passwords = ['plain' => 'secret', 'dash' => '-', 'spaces' => 'password with spaces', 'single quotes' => "password'with'quotes", 'double quotes' => 'password"with"quotes', 'assignment' => 'pass=word', 'dashes' => 'pass-with-dashes', 'shell metacharacters' => 'pass$with;chars', 'backticks' => 'pass`with`backticks', 'pipes' => 'pass&with|pipes', 'redirections' => 'pass>with<input', 'backslashes' => 'OCA\\Libresign\\Vendor\\pass\\with\\slashes', 'tabs' => "pass\twith\ttabs", 'latin accents' => 'senhaçã', 'cyrillic' => 'пароль', 'chinese' => '密码', 'arabic' => 'كلمةسر', 'emoji' => '🔐secret', 'emoji inside' => 'pass😊word', 'zero width joiner' => '👩‍💻🔑'];
        $cases = [];
    ...
    {
        return ['short option' => ['-tsp'], 'long option' => ['--tsa-password'], 'short option of another password' => ['-kp'], 'long option of another password' => ['--owner-password']];
    }
    ...
    {
        return ['short option' => [['-ksp' => 'secret']], 'long option' => [['--keystore-password' => 'secret']], 'short option with assignment' => [['-ksp=secret']], 'long option with assignment' => [['--keystore-password=secret']], 'without a value' => [['-ksp']]];
    }
