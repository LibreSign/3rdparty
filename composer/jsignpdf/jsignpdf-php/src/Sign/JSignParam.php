...
class JSignParam
{
    ...
    private const PASSWORD_OPTIONS = ['-kp' => '--key-password', '-opwd' => '--owner-password', '-upwd' => '--user-password', '-tscp' => '--tsa-cert-password', '-tsp' => '--tsa-password'];
    /**
    ...
    public function getJSignParameters() : string
    {
        ...
        foreach ($this->jSignParameters as $option => $value) {
            $parameters[] = \is_string($option) ? \escapeshellarg($option) . ' ' . \escapeshellarg($value) : \escapeshellarg($value);
        }
    ...
    {
        if (isset(self::CERTIFICATE_PASSWORD_OPTIONS[$parameter]) || \in_array($parameter, self::CERTIFICATE_PASSWORD_OPTIONS, \true)) {
    ...
    private function passwordOption(string $parameter) : ?string
    {
        ...
        $option = \array_search($parameter, self::PASSWORD_OPTIONS, \true);
