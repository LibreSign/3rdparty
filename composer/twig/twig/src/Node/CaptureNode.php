...
class CaptureNode extends Node
{
    ...
    public function compile(Compiler $compiler) : void
    {
        ...
        $compiler->raw($useYield ? "implode('', iterator_to_array(" : '\\OCA\\Libresign\\Vendor\\Twig\\Extension\\CoreExtension::captureOutput(')->raw("(function () use (&\$context, \$macros, \$blocks) {\n")->indent()->subcompile($this->getNode('body'))->write("yield from [];\n")->outdent()->write('})()');
        if ($useYield) {
            $compiler->raw(', false))');
        } else {
            $compiler->raw(')');
