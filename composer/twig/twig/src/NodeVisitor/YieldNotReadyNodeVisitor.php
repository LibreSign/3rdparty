...
final class YieldNotReadyNodeVisitor implements NodeVisitorInterface
{
    ...
    public function enterNode(Node $node, Environment $env) : Node
    {
        ...
        if (!($this->yieldReadyNodes[$class] = (bool) (new \ReflectionClass($class))->getAttributes(YieldReady::class))) {
            if ($this->useYield) {
                throw new \LogicException(\sprintf('You cannot enable the "use_yield" option of Twig as node "%s" is not marked as ready for it; please make it ready and then flag it with the #[\\OCA\\Libresign\\Vendor\\Twig\\Attribute\\YieldReady] attribute.', $class));
            }
            trigger_deprecation('twig/twig', '3.9', 'Twig node "%s" is not marked as ready for using "yield" instead of "echo"; please make it ready and then flag it with the #[\\OCA\\Libresign\\Vendor\\Twig\\Attribute\\YieldReady] attribute.', $class);
        }
        return $node;
    }
