    ...
    private function initFromAttributes() : void
    {
        ...
        foreach ($reflectionClass->getMethods() as $method) {
            ...
            foreach ($method->getAttributes(AsTwigTest::class) as $reflectionAttribute) {
                ...
                $attribute = $reflectionAttribute->newInstance();
                $callable = new TwigTest($attribute->name, [$reflectionClass->name, $method->getName()], ['needs_context' => $attribute->needsContext ?? \false, 'needs_environment' => $attribute->needsEnvironment ?? $this->needsEnvironment($method), 'needs_charset' => $attribute->needsCharset ?? \false, 'needs_is_sandboxed' => $attribute->needsIsSandboxed ?? \false, 'is_variadic' => $method->isVariadic(), 'deprecation_info' => $attribute->deprecationInfo]);
                if ($callable->getMinimalNumberOfRequiredArguments() > $method->getNumberOfParameters()) {
